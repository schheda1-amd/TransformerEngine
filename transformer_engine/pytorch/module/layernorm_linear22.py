...
class _LayerNormLinear(torch.autograd.Function):
    ...
    def forward(
        ...
        xcd_size: int,
        physical_gpu_offset: int,
    ) -> Union[Tuple[torch.Tensor, ...], torch.Tensor]:
        # pylint: disable=missing-function-docstring

        # NVTX label for profiling
        nvtx_label = "transformer_engine._LayerNormLinear22.forward"
    ...
    def backward(
        ...
        return (
            ...
            None,  # xcd_group
            None,  # xcd_size
            None,  # physical_gpu_offset
        )


class LayerNormLinear22(TransformerEngineBaseModule):
    r"""
    ...
    def __init__(
        ...
        self.physical_gpu_offset = physical_gpu_offset
        ...
        if xcd_group is not None:
            self.xcd_size = get_distributed_world_size(xcd_group)
            self.xcd_group = xcd_group
            self.physical_gpu_count = self.tp_size // self.xcd_size
        ...
        if self.parallel_mode == "column":
            ...
            # split like column parallel along "n" dimension, across physical GPUs
            # & split across "k" dimension for the same nth slice across 
        ...
        if not defer_init:
            ...
            for weight in self.weight_names:
                set_tensor_model_parallel_attributes(
                    ...
                    is_parallel=True,
                    dim=0 ,# 1 if self.parallel_mode == "row" else 0,
                    stride=1,
                )
    ...
    def forward(
        ...
        with self.prepare_forward(
            ...
            if self.parallel_mode == "bumblebee":
                weight_tensor, _ = gather_along_first_dim(weight_tensor, self.xcd_group, \
