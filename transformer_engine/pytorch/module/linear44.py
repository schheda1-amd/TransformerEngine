...
class _Linear(torch.autograd.Function):
    ...
    def forward(
        ...
        intra_xcd_group: Union[dist_group_type, None],
        intra_xcd_size: int,
        physical_gpu_count: int,
        inter_xcd_group: Union[dist_group_type, None],
        inter_xcd_size: int,
    ) -> torch.Tensor:
        ...
        if is_grad_enabled:
            ...
            ctx.intra_xcd_group = intra_xcd_group
            ctx.intra_xcd_size = intra_xcd_size
            ctx.physical_gpu_count = physical_gpu_count
            ctx.inter_xcd_group = inter_xcd_group
            ctx.inter_xcd_size = inter_xcd_size
        ...
        if ub_overlap_rs_fprop:
            ...
            nvtx_range_push(f"{nvtx_label}.bumblebee_comm")
            out, _ = allreduce(out, inter_xcd_group)
            nvtx_range_pop(f"{nvtx_label}.bumblebee_comm")
            # create the full matrix - for compatibility
            # check shapes
    ...
    def backward(ctx, grad_output: torch.Tensor) -> Tuple[Union[torch.Tensor, None], ...]:
        ...
        return (
            ...
            None,  # intra_xcd_group
            None,  # intra_xcd_size
            None,  # physical_gpu_count
            None,  # inter_xcd_group
            None,  # inter_xcd_size
        )


class Linear44(TransformerEngineBaseModule):
    ...
    def __init__(
        ...
        intra_xcd_group: Optional[dist_group_type] = None,
        ...
        if intra_xcd_group is not None:
            self.intra_xcd_group = intra_xcd_group
            self.intra_xcd_size = get_distributed_world_size(intra_xcd_group)
            self.physical_gpu_count = self.tp_size // self.intra_xcd_size
            self.inter_xcd_group = inter_xcd_group
            self.inter_xcd_size = get_distributed_world_size(inter_xcd_group)
        ...
        if self.parallel_mode == "column":
        ...
        elif self.parallel_mode == "bumblebee":
            self.out_features = divide(self.out_features, self.intra_xcd_size)
            self.in_features = divide(self.in_features, self.physical_gpu_count)
            ...
            #        device=device,
            #        dtype=params_dtype,
            #        ) for _ in range(self.intra_xcd_size)
            #]
    ...
    def forward(
        ...
        with self.prepare_forward(
            ...
            args += (
                ...
                self.intra_xcd_group,
                self.intra_xcd_size,
                self.physical_gpu_count,
                self.inter_xcd_group,
                self.inter_xcd_size,
            )
            out = linear_fn(*args)
